...
@section('title', 'Modifica ' . $cantiere->nome . ' - Gestionale Cantieri')

@section('content')
<div class="max-w-3xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('cantieri.show', $cantiere) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
            <i class="fas fa-arrow-left mr-2"></i> Torna al cantiere
        </a>
        <h1 class="text-3xl font-bold text-gray-900 mt-4">Modifica Cantiere</h1>
        <p class="mt-2 text-sm text-gray-600">{{ $cantiere->codice }}</p>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-lg shadow-lg p-6">
        <form action="{{ route('cantieri.update', $cantiere) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text"
                       name="committente"
                       id="committente"
                       value="{{ old('committente', $cantiere->committente) }}"
                       required
            ...
            <div>
                ...
                <textarea name="indirizzo"
                          id="indirizzo"
                          rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('indirizzo') border-red-500 @enderror">{{ old('indirizzo', $cantiere->indirizzo) }}</textarea>
                @error('indirizzo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date"
                           name="data_inizio"
                           id="data_inizio"
                           value="{{ old('data_inizio', $cantiere->data_inizio?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date"
                           ...
                           value="{{ old('data_fine_prevista', $cantiere->data_fine_prevista?->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <input type="number"
                       name="importo_lavori"
                       id="importo_lavori"
                       value="{{ old('importo_lavori', $cantiere->importo_lavori) }}"
                       step="0.01"
                       min="0"
            ...
            <div>
                ...
                <select name="stato"
                    ...
                    <option value="pianificazione" {{ old('stato', $cantiere->stato) === 'pianificazione' ? 'selected' : '' }}>Pianificazione</option>
                    <option value="apertura" {{ old('stato', $cantiere->stato) === 'apertura' ? 'selected' : '' }}>Apertura</option>
                    <option value="attivo" {{ old('stato', $cantiere->stato) === 'attivo' ? 'selected' : '' }}>Attivo</option>
                    <option value="sospeso" {{ old('stato', $cantiere->stato) === 'sospeso' ? 'selected' : '' }}>Sospeso</option>
                    <option value="completato" {{ old('stato', $cantiere->stato) === 'completato' ? 'selected' : '' }}>Completato</option>
                    <option value="chiuso" {{ old('stato', $cantiere->stato) === 'chiuso' ? 'selected' : '' }}>Chiuso</option>
                </select>
                @error('stato')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="note"
                          id="note"
                          rows="4"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('note') border-red-500 @enderror">{{ old('note', $cantiere->note) }}</textarea>
                @error('note')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
